...
### 【重要】従来のVC構成との違い：「鍵のインポート」が存在しない

ここまでの説明で疑問に思うかもしれません。第1部・第2部では、鍵（keystore）を
「VC（Validator Client）」にインポートする工程がありました。
...
**SSVでクラスター運用まで進めると、この「インポート」という工程自体が
存在しないことに気づきます。** 鍵はSSV App上で複数の「KeyShare」に分割され、
各オペレーターのSSVノードに配布されます。元の単一の鍵は、分割が完了した
時点で誰の手元にも単独では存在しなくなります。

```
従来：あなたの鍵 → import → あなたのVC（1プロセス） → 署名
SSV ：あなたの鍵 → 4分割（Generate Key Shares） → 4人のSSVノードに配布
       → 4つの部分署名をQBFT/IBFTコンセンサスで合算 → 1つの署名
```

> 💡 「VCへのインポート」に相当するSSV独自の手順は、SSV Appでの
> 「Generate Key Shares → クラスター登録トランザクション」です。
> 手順の見た目（ファイルをアップロードする等）は似ていても、
> 行われている処理の本質は全く異なります。
>
> 実際に4人クラスターを組んで3-of-4閾値署名まで動かした検証記録と、
> 従来構成との対比図は[第4部・応用編 第6章](../第4部_応用編/第6章_SSV_DVTクラスター実践検証.md)
> で詳しく解説しています。

### 【補足】「二毛作（兼業）」と「DVT（分散）」は別の話

本書冒頭の「二毛作」の説明と、ここまでのDVTの説明を、混同しないように
整理しておきます。両者は全く別のレイヤーの話です。

```
二毛作（本書の構成）：
  1台のベアメタルサーバが、2つの「役割」を兼業している
    役割A：Lido CSMバリデータ（第1部・第2部で構築）
    役割B：SSVオペレーター（本パートで追加）
  → これは「1台のマシンに2つの仕事をさせている」という、
    リソース活用の話

DVT（本パートのSSV）：
  1つの「バリデータの鍵」が、複数台（最低4台）に分散している
  → これは「1つの鍵の責任を、複数のマシン・複数の運用者に
    分散させている」という、可用性・耐障害性の話
```

つまり、今回構築したSSVノードは「あなたのベアメタル1台」という意味では
集中（二毛作で兼業）していますが、それが本物のクラスターに組み込まれると、
**そのバリデータ自体の運用は、あなたのベアメタルを含む4台に分散**します。

```
あなたのベアメタル1台
  ├─ 役割A：Lido CSMバリデータ（このマシン単独で署名・単一障害点）
  └─ 役割B：SSVオペレーター（4台中の1台。他の3台と組んで初めて
             1つのSSVバリデータの署名が成立する）
```

> ⚠️ **役割Aと役割Bは、全く別のバリデータ（別の鍵）を担当している点に注意。**
> 役割Aのバリデータは、このマシンが落ちればそのまま単一障害点になります
> （二毛作はリソース効率の話であり、Aの耐障害性を上げるものではありません）。
> 役割Bのバリデータは、このマシンが落ちても残り3台（閾値次第）で
> 署名が継続します（これがDVTの効果）。
>
> 「二毛作で2つの仕事をしている」ことと「DVTで1つの仕事を分散している」
> ことは、別軸の話として理解しておくと混乱しません。
...
## 【実録】SSV v2.4.2 + Lido CSM(Hoodi) 環境でのネットワーク名競合とハイブリッド構成の確立

構築時に遭遇したトラブルと、試行錯誤の末に確立した回避策の記録です。
公式ドキュメントに記載のない構成のため、SSVのバージョンアップで状況が変わる可能性があります。

なお、本構成はSSVノードの**起動・同期・ネットワーク接続まで検証済み**ですが、
クラスター運用（仲間のオペレーター3人が必要）は未実施のため、
実際のKeyShare署名・DVT稼働については今後の検証課題です。
...
| SSVノードの起動・同期確認 | パニックを回避し、1スロットの遅延もなく最新チェーンヘッドを追従（P2P: 3ピア接続確認） |
| 回避策の記録 | 公式ドキュメントに記載のない構成の試行錯誤の記録。バージョンアップで状況が変わる可能性あり |
| **今後の課題** | クラスター結成（オペレーター3人募集中）→ KeyShare署名 → DVT実運用の検証 |
...
- **第3部：** 余剰リソースを活かしたSSV(DVT)オペレーター登録・ハイブリッド構成の確立（クラスター運用は仲間のオペレーター募集中）

> ✅ **ソロステーキング（Lido CSM）で10バリデータを安定稼働させながら、SSV v2.4.2 のネットワーク名非対応問題を自力で発見・特定し、試行錯誤の末にハイブリッド構成を確立しました。SSVノードの起動・同期・P2P接続まで検証済みです。クラスター運用（DVT実運用）はオペレーター仲間の募集から始まります。公式ドキュメントに記載のない構成のため、今後のバージョンアップで状況が変わる可能性があります。**
...
| 日付 | 実績 |
|---|---|
| 2026年 | **Lido CSM ICS（Identified Community Stakers）合格** |
| 2026年 | **ブロックプロポーザル 4回成功・失敗ゼロ（Proposal Efficiency 98.6%）** |
| 2026年 | **BeaconScore 97.37% / Attestations 161,647回 / Slashings 0** |

> 💡 **ICSとは：** Lido CSMの中でも審査・選考を通過した信頼できるオペレーターとして公式認定される制度です。ゼロベースからブロックチェーンインフラに挑戦し、構築・運用・トラブル解決の全過程をこの3部作に記録しました。
