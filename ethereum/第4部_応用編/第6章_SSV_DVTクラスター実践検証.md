# Ethereum バリデータ構築 第4部・応用編 第6章 SSV DVTクラスター実践検証

> **第3部で構築したSSVオペレーターを使い、実在する他の運用者3者と
> 組んで、本物の4-of-4分散バリデータ（DVT）クラスターを構築する検証**
...
| 検証環境 | テストネット用VM + SSV検証専用ウォレット |
| 検証日 | 2026年6月（Hoodi Testnet） |
| 進行状況 | 完了。クラスター登録 → Activation → 4-of-4閾値署名の継続安定稼働まで実証済み |
...
> 刻まれており、本物の4-of-4クラスターによる閾値署名（partial_signature
