...
> 💡 **32 ETHではなく約1.3 ETHのみで登録できました。**
> これはLido CSMの「ボンドカーブ」の仕組みによるものです。
> 本検証環境はDefault（DEF）タイプのオペレーターとして登録されており、
> 公式パラメータ（`csm.lido.fi/type/parameters`）では以下のボンド額が定められています。
>
> | Operator Type | 1本目の鍵 | 2本目以降 |
> |---|---|---|
> | Default（DEF・本検証環境） | 2.4 ETH | **1.3 ETH** |
> | Identified Community Staker（ICS） | 1.5 ETH | 1.3 ETH |
>
> 今回は5本目の鍵（index 4）の追加だったため「2本目以降」の単価が適用され、
> 実測1.298 ETHとほぼ一致しました。残りの大部分（約30.7 ETH）は
> Lidoプロトコル側が補填する仕組みです（第1部参照）。
>
> ⚠️ **本番のベアメタル環境（Operator #406）はICS認定済みですが、
> 本検証環境（テストネット・Operator #452）はDEFのままです。**
> ICSとDEFでボンド額は異なるため、混同しないよう注意してください。
