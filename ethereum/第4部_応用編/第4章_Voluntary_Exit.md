...
| 検証日 | 2026年6月16〜17日（Hoodi Testnet） |
...
| 確認時のエポック | 102,486 |
| 引退可能エポック | 102,612 |
| 残りエポック数 | 102,612 - 102,486 = **126 epoch** |
| 1エポックの時間 | 約6.4分 |
...
sudo lighthouse account validator exit \
  ...
  --beacon-node http://127.0.0.1:5052 \
  --datadir /var/lib/lido-csm \
  --keystore /var/lib/lido-csm/validators/0xb6b94fc.../keystore-m_12381_3600_2_0_0-XXXXXXXXXX.json
```
...
### Lido CSM Keysダッシュボードでの確認（実録）

引退完了後、Lido CSMダッシュボード（`csm.testnet.fi/keys/view`）で
以下のステータス変化を確認しました。

| 鍵（短縮表示） | ステータス | Strikes | 備考 |
|---|---|---|---|
| `0x839cbc...` | **Active** | 0/3 | testnetcsm（index 3）継続稼働中 |
| `0x8137a7...` | Exited (withdrawal pending) + Slashed | 1/3 | 第2章スラッシング済み |
| `0x81ddbd...` | Exited (withdrawal pending) + Slashed | 1/3 | 第2章スラッシング済み |
| `0xb6b94f...` | **Exited (withdrawal pending)** | 0/3 | **今回のVoluntary Exit** ✅ |

> 💡 **`0xb6b94f...` の読み方：**
>
> | 項目 | 内容 | 意味 |
> |---|---|---|
> | Status | `Exited (withdrawal pending)` | 引退完了・残高引き出し待ち |
> | Slashed | なし | **正規引退の証拠** ✅ |
> | Strikes | 0/3 | ペナルティなし ✅ |
>
> 第2章のスラッシング済み鍵（`Slashed` タグあり）と比較すると
> **正規引退にはSlashedタグが付かない**ことが一目瞭然です。
> これがVoluntary Exitとスラッシングの最も分かりやすい違いです。
...
| 項目 | 変化 |
|---|---|
| Keys ステータス | Active → Exiting → Exited (withdrawal pending) |
| Bond | 引退完了後に返還 |
| ペナルティ | なし（正規引退のため） |
...
exited が確認できたらノードを安全に停止します。

> ⚠️ **`active_exiting` の間はノードを止めてはいけません。**
> exited を確認してから停止してください。

```bash
# サービスを停止する（VC → Lighthouse → Geth の順）
...
> 💡 **gethが `failed` と表示されるのは正常です。**
> `stop` コマンドで停止した場合の表示であり、問題ありません。

```bash
# 自動起動を無効化する（再起動後も起動しないように）
sudo systemctl disable geth lighthouse lighthouse-vc.service

# 確認
sudo systemctl is-enabled geth lighthouse lighthouse-vc.service
```
...
> ✅ **disabled が確認できればtestnetcsm2の引退作業は完全に完了です。**
...
| 知見 | 内容 |
|---|---|
| Voluntary Exitとスラッシングの違い | 正規引退はペナルティなし・Slashedタグなし・Exited記録 |
...
| exited確認後にノード停止 | active_exitingの間は止めてはいけない |
...
Claim方法：「All」を選択 →「Rewards Address」へ →「ETH」で受け取り
実行日：2026年6月19日夜
```
...
> 🔧 **【訂正】当初「stETHで受け取り、この時点で完了」と記録していましたが、
> これは誤りでした。** 実際には「ETH」を選択していたため、CSM UIでの
> Claim操作は「出金リクエストの発行（NFTのミント）」まで進んだだけで、
> stETH・ETHのいずれも、まだウォレットには届いていませんでした。この
> リクエストは47日間、未完了のまま放置されていたことが、第7章の別の
> 検証（未稼働検証のBond Claim）の過程で偶然発覚し、2026年8月6日に
> ようやく最終Claim（stake-hoodi.testnet.fi）を実行し、実際にETHが
> 着金しました。詳しい経緯と、Bond ClaimがなぜNFT経由になるのかという
> 仕組みの解説は、第7章8-6節にまとめています。
>
> **実用的な教訓：** CSM UIで「Claim」操作を行っただけでは完了して
> いない場合があります。特に「ETH」を選んだ場合は、CSM UIとは別の
> サイト（Lido本体の出金窓口）で、もう一段階の最終Claim操作が必要です。
> 「操作した＝資金が届いた」と思い込まず、必ずウォレットの残高や
> Etherscanの実際の入金記録で裏を取る習慣が重要です。
...
⑤ 【別途・手動操作】csm.testnet.fi/bond/claim で
   Excess Bond（超過担保分）をclaim申請する
        ↓（ETHを選んだ場合のみ発生：出金キューでの待機。
           今回は結果的に47日間未完了のまま放置）
⑥ 【さらに別途・手動操作】stake-hoodi.testnet.fi（Lido本体の
   出金窓口）の「Claim」タブで、最終的にETHを受け取る
```

> 💡 **③〜④はネットワークが自動的に処理しますが、⑤⑥は自分で操作が
> 必要な点を忘れないようにしましょう。** 特に⑤で「ETH」を選んだ場合は、
> ⑤の操作だけでは完了しておらず、⑥の最終Claimまで行って初めて資金が
> 手元に届きます。CSM UI側の「請求済み」という表示だけで安心せず、
> 実際にウォレットへ着金したかを確認するまでが、資金回収の完了です。
> （本ガイドでも、実際にこの⑥を47日間見落としていました。詳細は
> 8-3節の訂正、および第7章8-6節を参照してください。）
