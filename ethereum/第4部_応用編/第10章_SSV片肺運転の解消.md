...
### 4-5. 【追加検証】実際にAlchemyを止めてフェイルオーバーを確認する

複数Beacon Node構成が「設定上は正しい」ことは確認できましたが、
「実際にPrimary（Alchemy）が使えなくなった時、本当にSecondary
（Chainstack）へ自動的に切り替わるか」を、実機で検証しました。

**検証方法：** Alchemyダッシュボードの「Endpoints」設定にある、
ネットワーク単位の有効化/無効化機能を使い、Hoodiネットワークを
一時的に無効化しました。これはAPIキー自体を変更する操作ではなく、
そのAPIキーが「どのネットワークに対して使えるか」を制御する、
より軽量な操作です。

> 💡 **APIキーを再発行する方法（選択肢A）ではなく、ネットワーク単位
> の無効化（より軽量な方法）で代替できました。** これにより、検証後の
> 後処理（新しいキーをdocker-compose.yamlに反映する作業）が一切不要
> になり、安全性の高い検証ができました。

**タイムライン（実測）：**

```
09:26:39  CL Status : 2/2 synced（正常、検証前のベースライン）

（AlchemyダッシュボードでHoodiネットワークを無効化）

09:29:26  CL Status : 1/2 synced (degraded, check CL Source!)
          Validator : attesting（継続中、署名は止まっていない）

（Alchemyダッシュボードでネットワークを再度有効化）

09:37:27  CL Status : 1/2 synced (degraded)（まだ復帰前）
09:39:44  CL Status : 2/2 synced（自動的に正常復帰）
```

> ✅ **これは、単一障害点解消の効果が実際に機能することを示す、
> 決定的な証拠です。** Alchemyが使えなくなった約3分後には
> 「degraded」状態が検知され、それでも`Validator: attesting`は
> 一度も途切れませんでした。Chainstack（Secondary）が、Alchemy
> 不在の間も署名業務を支え続けていたことになります。

> 💡 **復帰についても、手動操作は一切不要でした。** Alchemy側で
> ネットワークを再度有効化してから、約2分後に自動的に「2/2 synced」
> へ復帰しました。SSVノードは、一度切断されたエンドポイントも
> 定期的に再チェックし、復活すれば自動的に再参加する仕組みを
> 持っていることが確認できました。`docker compose up -d`のような
> 手動の再起動操作は、フェイルオーバーの検知・復帰のどちらにも
> 必要ありませんでした。

**もう一つの裏付け：Chainstack側のリクエスト数スパイク**

`node_check.sh`の記録とは別に、Chainstackのダッシュボード自身にも、
フェイルオーバーが発生した痕跡がそのまま残っていました。

```
通常時：1秒あたり数件〜十数件程度の、低く安定したリクエスト数
        （Secondaryとして、ほとんど使われていない状態）
フェイルオーバー発生時：60件近くまで急激なスパイク
```

```
Total requests（直近6時間）: 1,693
Successful requests: 99.9%
Invalid requests: 1
```

> ✅ **これは「テキストログ上の状態変化」とは別の、独立した角度
> からの証拠です。** Alchemyが使えなくなった瞬間、SSVノードからの
> 全リクエストが一時的にChainstackへ集中したことが、Chainstack自身の
> トラフィックグラフという、ノードの外側からの記録によっても裏付け
> られました。複数の独立した記録（`node_check.sh`の状態遷移、
> Validatorのattesting継続、Chainstack側のトラフィック増加）が
> すべて一致したことで、フェイルオーバーが実際に機能したことを、
> 高い確度で確認できました。

> ⚠️ **Grafanaダッシュボードの限界も、同時に見つかりました。**
> `node_check.sh`が「1/2 synced (degraded)」を正確に検知していた
> のに対し、Grafanaの「SSV CL Status」パネルは、片肺状態の間も
> 単純に「Synced」（緑）と表示し続けていました。これは、Grafana側の
> クエリが`ssv_cl_sync_status{...="synced"}`という、「syncedの系列が
> 1つでも存在するか」を見る単純な作りだったためです。`node_check.sh`
> では「synced数 / 全体数」という比率で集計するよう既に修正済み
> でしたが、Grafana側のクエリはまだ単一エンドポイント前提のままで
> あることが、今回の実機検証で明らかになりました。これは今後の
> 課題として記録します。

### 4-6. 【追加発見】再起動直後の「1/1 synced（緑）」という、もう一つの落とし穴

別の検証（クラスターの3-of-4閾値冗長性の実証、第6章参照）に伴い、
SSVノードを一時停止・再開した際、再開直後の`node_check`で、以下の
ような表示が出ました。
...
これは緑色（正常）で表示されますが、本来は「2/2 synced」になるべき
状態です。コンテナの再作成直後、2本設定しているうちの1本（Chainstack）
との接続が、まだ確立していない過渡的な状態でした。約3分後、再度
確認すると「2/2 synced」「P2P Peers: 6」に復帰しました。

> ⚠️ **「synced数 / 全体数」が一致していれば緑になる、という
> node_check.shの判定ロジックには、見落としがあったことが分かり
> ました。** 「2/2」と「1/1」は、どちらも「全部synced」という意味
> で緑になりますが、後者は「本来あるべき母数（2）自体が、何らかの
> 理由で見えなくなっている」状態です。色だけで判断せず、表示されて
> いる数字（全体数）まで確認する必要があるという、運用上の教訓です。

> 💡 **これは4-5で発見した「degraded」のケースとは、別の種類の
> 落とし穴です。** 4-5は「全体数（2）は正しいが、syncedの数が
> 足りない（1/2）」というケースを正しく検知できていました。一方、
> 今回発見したのは「全体数自体（本来は2のはずが、一時的に1にしか
> 見えない）が縮小してしまう」ケースです。`grep`でカウントしている
> `SSV_CL_TOTAL_COUNT`が、再起動直後の過渡的な状態では、まだ片方の
> エンドポイントのメトリクスが出力されておらず、結果的に「1」に
> なってしまうことが原因と考えられます。両者を区別して検知する
> ロジックの改善は、引き続き今後の課題とします。
...
| フェイルオーバーは実機で機能した | Primary（Alchemy）を約10分間使えなくしても、Secondary（Chainstack）が署名業務を支え続け、Validatorのattesting状態は一度も途切れなかった |
| 複数の独立した記録による裏付け | `node_check.sh`の状態遷移・Validatorのattesting継続・Chainstack側のトラフィック増加という、3つの異なる角度からの記録が一致したことで、フェイルオーバーの実証に高い確度が得られた |
| 復帰も完全に自動 | Alchemy復旧後、約2分で「2/2 synced」に自動復帰した。手動でのコンテナ再起動・再作成は、検知・復帰のどちらの場面でも不要だった |
| 監視ダッシュボードにも見直しの余地が残った | `node_check.sh`は複数エンドポイント対応済みだったが、Grafanaの「SSV CL Status」パネルは旧来のクエリのままで、片肺状態を検知できていなかった |
| 「緑」には2種類の落とし穴がある | 「synced数が母数に対して不足（1/2）」と「母数自体が一時的に縮小（1/1）」は、どちらも現在のロジックでは緑判定になりうる。色だけでなく、表示された数字（全体数）まで確認する必要がある |
...
```
[ ] Grafanaの「SSV CL Status」パネルのクエリを、node_check.shと
    同様に「synced数 / 全体数」を集計する形に修正する
[ ] node_check.shのCL Status判定に、「母数（SSV_CL_TOTAL_COUNT）
    自体が、本来あるべき数より少なくなっていないか」を検知する
    ロジックを追加する（再起動直後の過渡的な「1/1 synced」を、
    「全部synced」とは別の状態として区別できるようにする）
[ ] SSV専用CLクライアント「Anchor」の動向を継続的に確認
[ ] v2.5.x登場時に、ローカルLighthouse接続が可能になっているか再検証
[ ] クラスター相手3社（Operator A・B・C）の定期確認ルーチンの確立
[ ] 【新規】外部RPCプロバイダーの無料枠消費率を、Grafana Alertingの
    監視対象に加える
    - 現状、Alchemy（約21%消費）・Chainstack（ほぼ未使用）ともに
      余裕があるが、「無料枠を使い切って、フェイルオーバー先も
      想定外に機能停止する」というシナリオは、今のところ
      事前に検知できない
    - インフラの死活監視（第9章）に加えて、「外部サービスの
      利用枠」という、もう一段上のレイヤーの監視が必要
[ ] 【理想形の再確認】本来「セカンダリ（保険）」であるべき外部RPCが、
    現状ではPrimary（主役）になっている逆転状態
    - 理想：EL（ローカルGeth）+ CL Primary（ローカルLighthouse）+
      CL Secondary（外部RPC、保険専用）
    - 現実：EL（ローカルGeth）+ CL Primary（外部Alchemy）+
      CL Secondary（外部Chainstack）
    - v2.4.2のdataラッパー問題（セクション2-1）が解消されれば、
      自然とこの理想形に近づく。ランニングコスト・人的コストを
      最小化する観点でも、ローカルCLを主役にする方向性が望ましい
```
