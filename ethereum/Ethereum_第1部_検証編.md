...
> ⚠️ **EFIの有効化はOSインストール前に必ず行うこと。**
> VirtualBox の「設定 → システム → マザーボード」タブを開き、
> 「EFIを有効化（特定のOSのみ）」に **必ずチェックを入れてから** インストールを開始してください。
>
> **なぜインストール前でなければならないか：**
> EFIを無効のままOSをインストールすると、
> Ubuntu側に「古い起動方式（BIOS）」用のブート情報が書き込まれます。
> 後からEFIを有効化すると「EFIモードでUbuntuが見つからない」
> ブートミスマッチが発生し、VMが起動不能になります。
> NVMeストレージ + EFI有効 + インストール の組み合わせが最も安定します。
>
> 正しい手順：
> 1. VMを電源オフにする
> 2. 設定 → システム → マザーボード → 「EFIを有効化」にチェック
> 3. 設定 → ストレージ → ISOイメージがセットされているか確認
> 4. VMを起動してインストール開始
>
> （EFI無効でインストールしてしまった場合は作り直しが必要です）
...
> ⚠️ **必ず公開鍵でSSH接続できることを確認してから実施してください。**
> パスワード認証を無効にした後に公開鍵認証が失敗すると
> サーバーに入れなくなります（VMなのでコンソールから復旧可能ですが）。
...
> 💡 **viエディタの操作：**
> `i` で入力モード → 編集 → `Esc` → `:wq` で保存終了

以下の項目を変更・確認する：

- パスワード認証を無効化（公開鍵認証のみ許可）
- rootでのSSHログインを禁止
- 公開鍵認証を有効化（デフォルトでyesだが明示的に設定）
...
# 別のターミナルで接続確認（現在のセッションは維持される）
# → 新しいターミナルで ssh -p 2222 <your_user>@127.0.0.1
# → 接続できればOK
```
...
> | 認証方式 | 安全性 | 理由 |
> |---|---|---|
> | パスワード認証 | ❌ 危険 | ブルートフォース攻撃（自動で何万回も試す）に弱い |
> | 公開鍵認証 | ✅ 安全 | 正しい鍵ペアがなければ数学的に侵入不可能 |
>
> インターネットに公開されたサーバーは
> 常時攻撃を受けています。
> パスワード認証を残しておくのは
> 玄関のドアを半開きにしているようなものです。

> 💡 **`sshd_config` の編集時の注意：**
> `#PasswordAuthentication yes` のように
> `#` でコメントアウトされている行は無効です。
> `#` を削除して `no` に変更することを忘れずに。
...
```bash
cat ~/csm-artifacts/ethstaker_deposit-cli-*-linux-amd64/validator_keys/deposit_data-*.json | jq '.[] | {
  network_name,
  withdrawal_credentials,
  pubkey
}'
...
> 💡 **なぜディレクトリ移動またはフルパスが必要か：**
> `cat validator_keys/...` はカレントディレクトリからの
> 相対パスで指定しています。
> 別のディレクトリにいると「そんなファイルはない」エラーになります。
> `cd` で移動するか、`~/` から始まるフルパスで指定してください。
...
表示された `0x...` から始まる文字列が、あなたのバリデータの公開鍵です。
これをbeaconcha.inの検索欄に貼り付けて確認します。
...
> ⚠️ **要注意：URLの先頭「hoodi.」を必ず確認してください**
>
> `hoodi.` を付け忘れて `beaconcha.in`（メインネット版）で
> 検索してしまうと、意図しないバリデータの情報が表示されて
> しまうことがあります。特にインデックス番号で検索した場合、
> メインネットとHoodiでは番号が別々にカウントされているため、
> 全く別人（メインネットの実在バリデータ）のページが
> 表示されてしまいます。
>
> **見分け方：**
> - ブロック提案履歴が「〇〇日前」と大きく古い日付になっている
> - Total Rewardsが数ETH単位ですでに溜まっている
>
> これらが出た場合はメインネット版を見ている可能性が高いです。
> 自分のバリデータは作ったばかりのはずなので、履歴が浅いのが正常です。
>
> 💡 公開鍵（pubkey）は世界に1つしかない値なので、
> インデックス番号よりも公開鍵で検索する方が確実です。
