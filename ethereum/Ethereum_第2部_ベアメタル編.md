...
### 鍵データの扱いで出てくる「一時的な場所」について

第2部では鍵データを扱う場面が複数あります。
それぞれ異なる方法を使うため、ここで整理しておきます。

| 場面 | 問題 | 解決方法 | 後処理 |
|---|---|---|---|
| VMからのエクスポート | `.sqlite` を直接コピーすると破損する | `lighthouse export` で `/tmp` にJSON化 | 転送後に `rm` で削除 |
| 物理PCへの転送 | `/var/lib/lido-csm/` はethereum所有で書けない | 一時的に `$USER` 所有に変更してscp | 転送後に `chown ethereum` に戻す |
| VCへのインポート | `/home/<your_user>/` は700の壁で読めない | `/tmp/keys_import/` 経由でimport | import後に `rm -rf` で削除 |

> 💡 **共通の原則：**
> 鍵データは「必要な時だけ一時的な場所に置き、用が済んだらすぐ削除・元に戻す」
> これがセキュリティを保ちながら作業する基本です。
...
> 💡 **これは、サーバー（ベアメタルPC）の中の設定ではなく、自宅のルーター側の設定です。**
> 多くのステーキングガイドは、ここを「もう設定済み」という前提で省略していますが、
> 初めて自宅サーバーを公開する方にとっては、最初につまずきやすい部分です。
> 順番に、焦らず進めましょう。

### なぜルーター設定が必要なのか

```
あなたのサーバー（PC）は、自宅のルーターを経由して、
インターネットに接続されています。

ルーターは、自宅内の複数の機器（PC、スマホ、TV等）を、
1つの「インターネットへの入り口」で束ねる役割をしています。

→ そのため、外部（インターネット上の、他のEthereumノード）から、
  あなたのサーバーに直接アクセスするには、ルーターに対して、
  「このポートへの通信は、このサーバーに渡してください」という、
  道案内の設定をしてあげる必要があります。
```

### 1. プライベートIPアドレスとMACアドレスについて

```
プライベートIPアドレス：
  自宅のネットワーク内だけで使われる、機器を識別するための番号
  （例：192.168.1.10 のような形式）
  → インターネット全体で見える、グローバルIPアドレスとは別物
  → 自宅内の機器それぞれに、ルーターが自動的に割り振っている

MACアドレス：
  ネットワーク機器（PCのLANポートやWi-Fi子機）に、
  工場出荷時から付けられている、世界で重複しない固有の番号
  （例：AA:BB:CC:11:22:33 のような形式）
  → これは、機器を「物理的に」識別するための番号で、
    IPアドレスのように後から変わることはない
```

```bash
# サーバー（VMまたはベアメタル）側で、自分のMACアドレスを確認する
ip link show
# → 結果に表示される「ether」の後ろの文字列がMACアドレス
# 例：ether 08:00:27:aa:bb:cc
```

### 2. DHCPでのIPアドレス予約（固定IP化）

```
通常、ルーターは「DHCP」という仕組みで、機器が接続するたびに、
自動的にプライベートIPアドレスを割り振っています。

このままだと、サーバーを再起動した際などに、
IPアドレスが変わってしまう可能性があります（例：192.168.1.10 → 192.168.1.15）。

→ ポートマッピングの設定（次の節）は、特定のIPアドレス宛てに
  固定で行うため、サーバーのIPアドレスが変わってしまうと、
  ポートマッピングの設定ごとやり直しになってしまいます。

そこで、「このMACアドレスの機器には、いつも同じIPアドレスを
割り振ってください」と、ルーターに予約しておきます。
これを「DHCP予約」または「IPアドレスの固定割り当て」と呼びます。
```

```
設定手順（一般的な流れ。表記や場所はメーカー・機種によって異なります）：
  ① ルーターの管理画面にアクセスする
     （ブラウザで 192.168.1.1 や 192.168.0.1 等、
      ルーターのIPアドレスを入力。ルーター本体の底面シール等に
      記載されていることが多い）
  ② 「DHCP設定」「アドレス予約」「IPアドレス固定割り当て」
     等の名称のメニューを探す
  ③ サーバーのMACアドレスを入力し、割り当てたいIPアドレス
     （例：192.168.1.100）を指定する
  ④ 設定を保存し、サーバーを再起動してIPアドレスが
     固定されたか確認する
```

> ⚠️ **正確な操作手順は、お使いのルーターのメーカー・機種によって
> 大きく異なります。** 「（ルーターのメーカー名・型番） DHCP予約」
> 「（ルーターのメーカー名・型番） IPアドレス固定」等で検索し、
> メーカー公式のマニュアル・サポートページを確認してください。

### 3. ポートマッピング（ポートフォワーディング）

```
通常、ルーターは「外側（インターネット）から内側（自宅内）への
通信」を、デフォルトでブロックしています。これは、不正アクセスを
防ぐための、基本的な防御の仕組みです。

ただし、「内側から外側へ」自分が接続しに行く通信（Outbound）は、
特に設定しなくても、最初から通ることができます。
```

```
ポートが閉じている状態：
  あなたのノードは、自分から外部のノードに接続することはできる
  （Outbound：内側 → 外側）

  しかし、外部のノードが、あなたのノードに接続してくることは
  できない（Inbound：外側 → 内側がブロックされている）

ポートを開放（ポートマッピング）すると：
  外部のノードが、あなたのIPアドレス・指定したポート番号に向けて、
  直接接続できるようになる
```

```
なぜ開放した方がいいのか（効率の話）：
  自分から繋ぎに行ける相手（Outbound）だけでは、
  接続できるPeer（仲間のノード）の数に限りがあります。

  ポートを開放すると、外部から「あなたのノードに繋ぎたい」という、
  新しい相手からの接続も受け入れられるようになり、Peer数が増えます。

  → Peer数が増えると、ブロックやAttestation情報の伝播が
    速く・安定するようになり、結果としてバリデータが
    タイムリーに仕事（Duty）を実行できる確率が上がります。
```

> 💡 **Peer数の目安については、node_check.shの「[4] ピア数」の
> セクションで詳しく扱います。** Peer数が十分に多ければ、
> ポート開放なしでも安定して運用できる場合があります。

#### 必要なポート番号一覧（本書のシリーズで使うもの）
...
Lighthouse（合意クライアント）:
  9000/TCP・UDP
  （上記2つは、本Phaseの「Step 9」でufwに登録済みの番号と同じです）

SSV Node（第3部・分散化編で使用）:
  12001/UDP（P2P探索用）
  13001/TCP（P2P通信用）
  ※UDPとTCPで、別々のポート番号を使う構成です。
  　具体的な番号は、第3部のdocker-compose.yaml設定に従ってください
```
...
設定手順（一般的な流れ）：
  ...
  ② 「ポートマッピング」「ポートフォワーディング」
     「静的IPマスカレード」等の名称のメニューを探す
  ③ 「外部ポート」「内部ポート」に、上記の番号を入力する
     （Geth: 30303、Lighthouse: 9000、SSV: 12001/UDP・13001/TCP）
  ④ 「転送先（宛先）IPアドレス」に、前節で固定したサーバーの
     IPアドレス（例：192.168.1.100）を指定する
  ⑤ TCP・UDPの両方を、それぞれ設定する
     （プロトコルの種類を選べる場合は両方にチェック。SSVは
     ポート番号自体がUDP/TCPで異なるため、特に注意が必要です）
  ⑥ 設定を保存する
```

> ⚠️ **このマニュアルでも、メーカー・機種ごとの正確な操作手順は
> 案内できません。** 「（ルーターのメーカー名・型番） ポート開放」
> 「（ルーターのメーカー名・型番） ポートフォワーディング」等で
> 検索し、メーカー公式のサポートページを確認してください。

### 4. セキュリティ上、知っておくべきこと（重要）

```
✅ ポートを開放しても、影響は「指定したIPアドレス・
   指定したポート番号」だけに限られます。

   例えば、Gethの30303番だけを、サーバーのIPアドレス
   （192.168.1.100）宛てに開放した場合：
   → 外部から直接アクセスできるのは、その192.168.1.100の
     30303番ポートだけです
   → 自宅内の、他の機器（PC、スマホ、NAS等）や、
     サーバーの他のポート（例：22番のSSH等）には、
     一切影響しません
   → 「ポートを1つ開けたら、家中の機器が無防備になる」
     ということは、正しく設定していれば起こりません
```

```
ただし、開放したポートの「先」（サーバー側）は、
きちんと守る必要があります：

✅ サーバー側のファイアウォール（ufw）で、本当に必要な
   ポートだけを許可していること（前節で既に設定済み）
✅ SSHのポートは、インターネット向けに開放しない（Tailscale
   経由のみアクセスを許可する設定を、すでに本Phaseの
   「Step 9」で実施済み）
✅ ルーターのファームウェアを最新に保つこと
   （ルーター自体の脆弱性対策）
```

> 💡 **「ポートを開ける」というと、漠然と「危険」というイメージを
> 持つ方が多いですが、実際には「特定の窓口を、特定の用途のために、
> 限定的に開ける」という、ピンポイントな操作です。** どの窓口
> （ポート番号）が、どの部屋（サーバー）に繋がっているかを正確に
> 把握していれば、過度に恐れる必要はありません。
...
第1部（VM）で作った鍵とデータを
物理PC（ベアメタル）へ安全に移行します。

この「引越し」には2種類のデータがあります：

| データ | 重要度 | 内容 |
|---|---|---|
| スラッシング保護DB | ★★★ 最重要 | 「どのブロックに署名したか」の履歴。これがないと二重署名のリスクが残る |
| バリデータ鍵 | ★★★ 最重要 | 署名に使う鍵本体。失うと運用不能 |
...
Step 2: スラッシング保護データをエクスポート
↓
理由：過去の署名履歴を引き継ぐことで
「このブロックはもう署名済み」という
保護機能が物理PCでも有効になる

Step 3: 鍵データをホストPC経由で物理PCへ転送
↓
理由：Tailscale VPN経由で暗号化して転送
公開インターネットに鍵を晒さない
```
...
┌─────────────┐         ┌──────────────┐         ┌─────────────┐
│  Ubuntu VM  │  scp    │  Windows PC  │  scp    │  物理PC     │
│ (VirtualBox)│ ──────► │ (ホストPC)   │ ──────► │ (ベアメタル) │
└─────────────┘  :2222  └──────────────┘Tailscale└─────────────┘

転送するもの：
① slashing_protection.json（署名履歴）
② validators/（バリデータ鍵ファイル群）
...
> | 停止順 | サービス | 理由 |
> |---|---|---|
> | 1番目 | lighthouse-vc（署名役） | 最初に止めることで二重署名リスクをゼロにする |
> | 2番目 | lighthouse（同期役） | VCが止まった後に停止 |
> | 3番目 | geth（実行層） | 最後に止める（DBのフラッシュに時間がかかるため） |
...
期待する出力：
drwx------ 4 ethereum ethereum 4096 ... /var/lib/lido-csm/validators
drwx------ 2 ethereum ethereum 4096 ... /var/lib/lido-csm/slashing_protection
...
> 💡 **`--num_validators` について：**
> 追加したい鍵の数を指定します。
> 現在1鍵を運用中で合計10鍵にしたい場合は `9` を指定します。
>
> | 現在の鍵数 | 目標 | 指定する値 |
> |---|---|---|
> | 1 | 10 | 9 |
